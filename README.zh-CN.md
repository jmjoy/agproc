...
| 对于不带 watch 的 dev server 来说，需要有一种手段能便捷地跑“重新编译 + 运行”流程，而且对人类和 AI 智能体来说都保证只有一个服务进程在运行 | `build-cmd` + `run-cmd` 分离并统一编排，构建失败有专属 marker 与**退出码 4**，AI 智能体无需解析文本；内置编排锁与生命周期管理，确保任何时候严格只有一个服务实例在运行，改完代码通过 `restart` 即可安全重跑 |
